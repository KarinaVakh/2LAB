...
public:
    ...
    Section(const Section& s) {// констурктор копирующий переносит все свойства из того обькта который передали  тут вручную скопируем все 3 поля
...
public:
  ...
  //1 вариант копирования
      p3 = s.p3; 
     //2 вариант копирования
        p3 = *new Point(s.p3);
    }
    ~Ray() { // диструктор
        delete &p3;
...
// для мув
   ...
   // если обьект одного и того же класса помещать в переменную своего или базовго класа
    ...
    {
        //поробуем создать пару отрезков
        Section* s1 = new Section;
        Section* s2 = new   Section(*s1);//второй отрезок создаем как копию первого (*s1) - разыменование указтеля
        delete s1;
        delete s2;
        // возникает ошибка мы удалили первый секшон а вторй не удалился нормально (обьяснить почему)
    } // теперь с ем же мэйном все нормально созд отрезок и 2 точки , созд второй отрезок путем костр коп и он созд 2 точки путем вызова из констр коп
   // при удален 1 - ого отр сначла уд его точки потом он сам и второй отрезок также
    ...
    {    
        cout << endl;
        Ray p; // задаем обьекты статически
        Ray p2(10, 20); // задаем обьекты статически
        Ray p3(p2); 
    }

    {
        ...
        cout << endl;
        Ray* p = new Ray; // задаем обьекты статически
        Ray* p = new Ray(10, 20); // задаем обьекты статически
        Ray* p2 = new Ray(p);
    }
